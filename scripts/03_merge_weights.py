...
def main():
    ...
    parser.add_argument("--adapter-path", type=str, default=None, help="Path to adapter checkpoint")
    parser.add_argument("--output-dir", type=str, default=None, help="Path for merged output")
    parser.add_argument("--save-method", type=str, default="merged_16bit", help="Unsloth save method")
    parser.add_argument("--dry-run", action="store_true", help="Perform dry run without loading model weights")

    args = parser.parse_args()
    merge_weights(args.variant, adapter_path=args.adapter_path, output_dir=args.output_dir, save_method=args.save_method, dry_run=args.dry_run)

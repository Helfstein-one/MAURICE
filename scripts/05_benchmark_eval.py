...
def run_benchmark_variant(variant: str, model_path: Optional[str] = None) -> Dict[str, Any]:
    if not model_path:
    ...
    if os.path.exists(model_path):
        for llama_cmd in ["llama-bench", "./llama-bench"]:
            try:
                res = subprocess.run([llama_cmd, "-m", model_path, "-n", "128", "-p", "512"], capture_output=True, text=True, timeout=10)
                if res.returncode == 0:
...
def main():
    ...
    parser.add_argument("--variant", choices=["c", "r", "g", "all"], default="all", help="Model variant to evaluate")
    parser.add_argument("--model-path", type=str, default=None, help="Path to GGUF model")
    parser.add_argument("--output-json", type=str, default="build/benchmark_results.json", help="Path to write JSON benchmark report")

...
class TestMauricePipeline(unittest.TestCase):
    ...
    def test_prepare_datasets(self):
        res = subprocess.run(["python3", "scripts/01_prepare_datasets.py", "--variant", "all", "--sample-size", "5"], capture_output=True, text=True)
    ...
    def test_train_qlora_dry_run(self):
        for v in ["c", "r", "g"]:
            res = subprocess.run(["python3", "scripts/02_train_qlora.py", "--variant", v, "--dry-run"], capture_output=True, text=True)
            self.assertEqual(res.returncode, 0, f"Error: {res.stderr}")
            self.assertTrue(os.path.exists(f"checkpoints/adapter_{v}/adapter_config.json"))

    def test_merge_weights_dry_run(self):
        for v in ["c", "r", "g"]:
            res = subprocess.run(["python3", "scripts/03_merge_weights.py", "--variant", v, "--dry-run"], capture_output=True, text=True)
            self.assertEqual(res.returncode, 0, f"Error: {res.stderr}")
            self.assertTrue(os.path.exists(f"checkpoints/merged_{v}/config.json"))

    def test_quantize_imatrix(self):
        for v in ["c", "r", "g"]:
            res = subprocess.run(["bash", "scripts/04_quantize_imatrix.sh", v], capture_output=True, text=True)
            self.assertEqual(res.returncode, 0, f"Error: {res.stderr}")
            self.assertTrue(os.path.exists(f"build/mau-llm-1.0-{v}-q4_k_m.gguf"))

    def test_benchmark_eval(self):
        res = subprocess.run(["python3", "scripts/05_benchmark_eval.py", "--variant", "all"], capture_output=True, text=True)
        self.assertEqual(res.returncode, 0, f"Error: {res.stderr}")
